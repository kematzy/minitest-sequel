...
describe Minitest::Assertions do
  describe '#refute_validates_min_length(:model, :attribute, :opts, :msg)' do
    before do
    ...
    describe 'should NOT raise an error' do
      it 'when column is NOT validated' do
        assert_no_error { refute_validates_min_length(@m, :author_id, 10) }
      end
    end
    ...
    describe 'should raise an error' do
      it 'when column is validated' do
        ...
        assert_error_raised(msg) { refute_validates_min_length(@m, :title, 10) }
      end

      it 'when trying to validate invalid column' do
        msg = /Column :does_not_exist is not defined in .*/

        assert_error_raised(msg) { refute_validates_min_length(@m, :does_not_exist, 10) }
      end
      ...
      it 'when no validations are defined for the model' do
        ...
        assert_error_raised(msg) { refute_validates_min_length(@m, :id, 10) }
      end
    end
    # /should raise an error
  end
  # /#refute_validates_min_length(:model, :attribute, :opts, :msg)
end
