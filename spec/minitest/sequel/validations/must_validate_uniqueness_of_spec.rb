...
describe Minitest::Expectations do
  describe '#must_validate_uniqueness_of(:attribute, :opts, :msg)' do
    before do
    ...
    describe 'should NOT raise an error' do
      it 'with valid unique Post#urlslug' do
        assert_no_error { _(@m).must_validate_uniqueness_of(:urlslug) }
      end
    end
    ...
    describe 'should raise an error' do
      describe 'when validating a valid option' do
        it ':message' do
          ...
          msg = /Expected .* to validate :uniqueness for :urlslug column with: { message: 'dummy' } but found: { message: 'is already taken' }/
          # rubocop:enable Layout/LineLength

          assert_error_raised(msg) do
            _(@m).must_validate_uniqueness_of(:urlslug, message: 'dummy')
          end
        end
      end
      ...
      describe 'when validating an invalid option' do
        %i[nil_message too_long too_short wrong_length].each do |t|
          it ":#{t}" do
            ...
            assert_error_raised(msg) do
              _(@m).must_validate_uniqueness_of(:urlslug, { t.to_sym => 'dummy' })
            end
          end
        end
        ...
        it ':is' do
          ...
          assert_error_raised(msg) do
            _(@m).must_validate_uniqueness_of(:urlslug, { is: 10 })
          end
        end
        ...
        it ':minimum' do
          ...
          assert_error_raised(msg) do
            _(@m).must_validate_uniqueness_of(:urlslug, { minimum: 10 })
          end
        end
        ...
        it ':maximum' do
          ...
          assert_error_raised(msg) do
            _(@m).must_validate_uniqueness_of(:urlslug, { maximum: 10 })
          end
        end
        ...
        it ':within' do
          ...
          assert_error_raised(msg) do
            _(@m).must_validate_uniqueness_of(:urlslug, { within: 10..20 })
          end
        end
      end
      ...
      it 'when the attribute is not being validated' do
        # rubocop:disable Layout/LineLength
        msg = /Expected .* to validate :uniqueness for :title column, but no validations are defined for :title/
        # rubocop:enable Layout/LineLength
      ...
      it 'when the attribute is not present' do
        ...
        assert_error_raised(msg) do
          _(@m).must_validate_uniqueness_of(:does_not_exist)
        end
      end
    end
    # /should raise an error
  end
  # /#must_validate_uniqueness_of(:attribute, :opts, :msg)
end
