...
describe Minitest::Expectations do
  describe '#wont_validate_length_of(:attribute, :opts, :msg)' do
    before do
    ...
    describe 'should NOT raise an error' do
      it 'when column is NOT validated' do
        assert_no_error { _(@m).wont_validate_length_of(:author_id) }

        assert_no_error { _(@m).wont_validate(:length, :author_id) }
      end
    end
    ...
    describe 'should raise an error' do
      it 'when column is validated' do
        ...
        assert_error_raised(msg) { _(@m).wont_validate_length_of(:title) }
      end

      it 'when trying to validate invalid column' do
        msg = /Column :does_not_exist is not defined in .*/

        assert_error_raised(msg) { _(@m).wont_validate_length_of(:does_not_exist) }
      end
      ...
      it 'when no validations are defined for the model' do
        ...
        assert_error_raised(msg) { _(@m).wont_validate_length_of(:id) }
      end
    end
    # /should raise an error
  end
  # /#wont_validate_length_of(:attribute, :opts, :msg)
end
