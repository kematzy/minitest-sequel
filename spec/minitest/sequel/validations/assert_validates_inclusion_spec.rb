...
describe Minitest::Assertions do
  describe '#assert_validates_inclusion(:model, :attribute, :opts, :msg)' do
    before do
    ...
    describe 'should NOT raise an error' do
      it 'when validating the inclusion of Post#title' do
        assert_no_error { assert_validates_inclusion(@m, :title, in: %i[a b c]) }
    ...
    describe 'should raise an error' do
      describe 'when validating a valid option' do
        ...
        it ':message' do
          ...
          assert_error_raised(msg) do
            assert_validates_inclusion(@m, :title, in: ['a'], message: 'dummy')
          end
        end
      end
      # /when validating a valid option
      ...
      describe 'when validating an invalid option' do
        %i[nil_message too_long too_short wrong_length].each do |t|
          it ":#{t}" do
            ...
            assert_error_raised(msg) do
              assert_validates_inclusion(@m, :title, in: ['a'], t.to_sym => 'dummy')
            end
          end
        end
        ...
        it ':is' do
          ...
          assert_error_raised(msg) do
            assert_validates_inclusion(@m, :title, in: ['a'], is: 10)
          end
        end
        ...
        it ':minimum' do
          ...
          assert_error_raised(msg) do
            assert_validates_inclusion(@m, :title, in: ['a'], minimum: 10)
          end
        end
        ...
        it ':maximum' do
          ...
          assert_error_raised(msg) do
            assert_validates_inclusion(@m, :title, in: ['a'], maximum: 10)
          end
        end
        ...
        it ':within' do
          ...
          assert_error_raised(msg) do
            assert_validates_inclusion(@m, :title, in: ['a'], within: 10..20)
          end
        end
      end
      ...
      it 'when the attribute is not being validated' do
        msg = /:inclusion for :author_id column, but no validations are defined for :author_id/

        assert_error_raised(msg) do
          assert_validates_inclusion(@m, :author_id, in: %i[a b c])
        end
      end
      ...
      it 'when the attribute is not present' do
        ...
        assert_error_raised(msg) do
          assert_validates_inclusion(@m, :does_not_exist, in: %i[a b c])
        end
      end
    end
