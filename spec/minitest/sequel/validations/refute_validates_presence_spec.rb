...
describe Minitest::Assertions do
  describe '#refute_validates_presence(:model, :attribute, :opts, :msg)' do
    before do
    ...
    describe 'should NOT raise an error' do
      it 'when column is NOT validated' do
        assert_no_error { refute_validates_presence(@m, :author_id) }
    ...
    describe 'should raise an error' do
      it 'when column is validated' do
        ...
        assert_error_raised(msg) { refute_validates_presence(@m, :title) }
      end

      it 'when trying to validate invalid column' do
        msg = /Column :does_not_exist is not defined in .*/

        assert_error_raised(msg) { refute_validates_presence(@m, :does_not_exist) }
      end
      ...
      it 'when no validations are defined for the model' do
        ...
        assert_error_raised(msg) { refute_validates_presence(@m, :id) }
      end
    end
    # /should raise an error
  end
  # /#refute_validates_presence(:model, :attribute, :opts, :msg)
end
