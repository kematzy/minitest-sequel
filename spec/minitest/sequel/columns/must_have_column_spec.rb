...
describe Minitest::Expectations do
  describe '#must_have_column(:attribute, :opts, :msg)' do
    let(:m) { Post.new }

    describe 'a valid column' do
      it { _(m).must_have_column(:title, type: :string, db_type: 'varchar(255)', allow_null: true) }

      describe 'raises no error' do
        it 'when given no options' do
          assert_no_error_raised { _(m).must_have_column(:title) }
        end

        it 'with options type: string' do
          assert_no_error { _(m).must_have_column(:title, { type: :string }) }
        end

        it "with options type: string, db_type: 'varchar(255)'" do
          assert_no_error_raised do
            _(m).must_have_column(:title, { type: :string, db_type: 'varchar(255)' })
          end
        end
        ...
        it 'with options default: nil' do
          assert_no_error { _(m).must_have_column(:title, { default: :nil }) }
        end

        it "with options default: '1'" do
          assert_no_error { _(m).must_have_column(:category_id, { default: '1' }) }
        end

        it 'with options max_length: 255' do
          assert_no_error { _(m).must_have_column(:title, { max_length: 255 }) }
        end

        it 'with options primary_key: :true' do
          assert_no_error { _(m).must_have_column(:id, { primary_key: true }) }
        end

        it 'with options auto_increment: :true' do
          assert_no_error { _(m).must_have_column(:id, { auto_increment: true }) }
        end
      end
      # /raises no error
      ...
      describe 'raises an error' do
        it "with incorrect options type: string, db_type: 'varchar(250)'" do
          ...
          assert_error_raised(msg) do
            _(m).must_have_column(:title, { type: :string, db_type: 'varchar(250)' })
          end
        end
        ...
        it 'with incorrect options allow_null: :false' do
          ...
          assert_error_raised(msg) { _(m).must_have_column(:title, { allow_null: false }) }
        end

        it 'with incorrect options max_length: 200' do
          msg = "Expected Post model to have column: :title with: { max_length: '200' } but found: "
          msg << "{ max_length: '255' }"

          assert_error_raised(msg) { _(m).must_have_column(:title, { max_length: 200 }) }
        end
        ...
        it 'with incorrect options primary_key: false' do
          ...
          assert_error_raised(msg) { _(m).must_have_column(:id, { primary_key: false }) }
        end

        it "with incorrect options primary_key: 'invalid'" do
          msg = /column: :id with: { primary_key: 'invalid' } but found: { primary_key: 'true' }/

          assert_error_raised(msg) { _(m).must_have_column(:id, { primary_key: 'invalid' }) }
        end
        ...
        it 'with incorrect options auto_increment: :false' do
          ...
          assert_error_raised(msg) { _(m).must_have_column(:id, { auto_increment: false }) }
        end

        it 'with incorrect options id: as string' do
          msg = "Expected Post model to have column: :id with: { type: 'string' } "
          msg << "but found: { type: 'integer' }"

          assert_error_raised(msg) { _(m).must_have_column(:id, { type: :string }) }
        end
        ...
        it 'when invalid option(s) are passed' do
          ...
          assert_error_raised(msg) { _(m).must_have_column(:id, { completey_invalid: false }) }
        end
      end
      # /raises an error
    end
    ...
    describe 'an invalid column' do
      it 'should raise error' do
        ...
        assert_error_raised(msg) { _(m).must_have_column(:does_not_exist) }
      end
    end
    # /an invalid column
  end
